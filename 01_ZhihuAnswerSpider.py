...
headers = {
    ...
    'cookie': r'_zap=b21c869d-8321-42a2-b50d-5a4e9488bebb; d_c0=AMDW8KRgSRiPTsjSIM6Sy6VQOkat7X8bdYE=|1710021142; q_c1=f1c2262db60448dabcbe1b146d4caa8f|1710021597000|1710021597000; _xsrf=c9rjxxRIYAKqLz2hBURMYsnFAZcWClTc; z_c0=2|1:0|10:1712152509|4:z_c0|80:MS4xd2o5RVJnQUFBQUFtQUFBQVlBSlZUYjJ0LW1ZV1NNRFphSlpXNjZFYlcwVXlTSi0yRTNNSUZnPT0=|7b11ce176060f16727da766f194848eabe5be0586d0ef2cd76d03b8bb4315dbb; Hm_lvt_98beee57fd2ef70ccdd5ca52b9740c49=1712316180,1712399711,1712476573,1712557308; tst=r; SESSIONID=MQpMr5Ru3DVqayCLVYtSJvNH8LP1fx1FLWNA49OmWeX; BAIDU_SSP_lcr=https://www.google.com/; JOID=UFsQAkpIsVrsFOIGLErDAVHnHWczBOYuoWXWUl149AqZdZhEZiT1S4kW5QQoS_qpsH0RftFUfB2IXSaeilxbW8w=; osd=W1sSBE1DsVjqE-kGLkzEClHlG2A4BOQopm7WUFt__wqbc59PZibzTIIW5wIvQPqrtnoaftNSexaIXyCZgVxZXcs=; Hm_lpvt_98beee57fd2ef70ccdd5ca52b9740c49=1712671365; KLBRSID=fb3eda1aa35a9ed9f88f346a7a3ebe83|1712671368|1712665308'
}
...
def answer_spider(v_result_file, v_question_id):
    ...
    while True:
        ...
        if os.path.exists(csv_file):  # 如果csv存在，不写表头，避免重复写入表头/If CSV exists, skip header
            header = False
        else:
            header = True
...
if __name__ == '__main__':
    # 问题id/Question ID(question_token)
    question_id = '366945849'
    # 保存文件名/Save CSV with the name of question ID
    csv_file = '知乎回答_{}.csv'.format(question_id)
    # 如果csv存在，先删除，避免由于追加产生重复数据 / Delete if exits, avoid duplicate
